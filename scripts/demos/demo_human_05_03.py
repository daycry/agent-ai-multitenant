...
_STATE_FILE = Path(__file__).resolve().parent / ".demo_state_05.json"
...
def main() -> int:
    ...
    print("\n  Que tienes que VER (sin hacer click):")
    print("\n    Card del agente 'HTTP Lookup Bot'")
    print("    (badge azul 'researcher', badge gris 'project_local').")
    print("    Dentro, fila para el tool 'example-weather' con:")
    print("       - badge azul   'http_endpoint'    (implementation_type)")
    print("       - badge verde  'safe'              (security_level)")
    print("       - texto        'timeout 10s . category data'")
    print("\n  Notas para entender lo que probo el script:")
    print("    - El allowlist concreto NO es un campo del Tool row;")
    print("      vive en la config del agent-runtime al boot. Por eso")
    print("      el step 1 lo prueba con un HttpEndpointTool construido")
    print("      in-process (no via la Tool row de la BD).")
    print("    - Cuando un agente real ejecute esta tool con URL fuera")
    print("      del allowlist, el error aparecera en /admin/executions/")
    print("      <id> (step_log) - ese cableado del agent loop a las")
    print("      Tool rows entra en un plan posterior.")
