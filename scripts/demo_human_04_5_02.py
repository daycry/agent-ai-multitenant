...
async def main() -> int:
    ...
    try:
        ...
        print(f"    · http://localhost:3000/admin/projects/{project_id}/knowledge-bases")
        print("        — verás dos KBs concedidas a este proyecto:")
        print(f"          origen ({kb_id})")
        print(f"          destino ({target_kb_id})")
        print(f"    · http://localhost:3000/admin/documents/{document_id}/citations")
        print("        — chunks del Document original con bounding boxes.")
        print(f"    · http://localhost:3000/admin/documents/{document_id}/ingestion")
        print("        — estado del pipeline (debe estar 'indexed').")
        print(f"    · http://localhost:3000/admin/documents/{promoted['document_id']}/citations")
        print("        — chunks del Document promovido a la KB destino.")
        print()
        print("  Nota: /admin/documents/<id> SIN sufijo da 404 — añade")
        print("  /citations o /ingestion. Cada hit del paso 1 lleva su cita")
        print("  kb_id + document_id; la UI usa ese par para abrir el viewer.")
        return 0
    finally:
