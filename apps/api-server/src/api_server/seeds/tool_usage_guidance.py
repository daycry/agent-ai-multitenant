...
BOTH_DOORS_ES = (
    ...
    "en tu workspace NO hay repositorio (no verás `.git`, y no falta nada); la "
    "plataforma versiona y comitea tu trabajo por ti al terminar. Si un comando "
    "responde «not found» o «command "
...
BOTH_DOORS_EN = (
    ...
    "(grep, ls, cat). Do NOT use `git` through either door: there is NO repository "
    "in your workspace (you will not see a `.git`, and nothing is missing); the "
    "platform versions and commits your work for you when you finish. If a command "
