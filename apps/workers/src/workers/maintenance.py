...
async def _reconcile_unpushed_worktrees(
    ...
    *,
    now: datetime,
    min_age: timedelta = _RECONCILE_STUCK_TASK_MIN_AGE,
) -> int:
    ...
    final PR is incomplete. ``_reconcile_stuck_tasks`` is blind to it (the task already
    left ``in_progress``). This pass finds terminal executions of OPEN plans whose diff
    is missing from the bare and pushes the surviving worktree (TTL 30d keeps it on
    disk). Age-gated (5 min settle) so it never competes with a worker still in
    post-processing, and it takes the SAME per-task run lock as the live path (A6) so
    backfill and a live commit can't corrupt the worktree in parallel. Returns how many
    were pushed. Best-effort per candidate; a real git error marks the execution
