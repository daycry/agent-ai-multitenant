...
- [x] **Test de migración up/down + invariantes de tabla**
  ...
  - ⚠️ **Dos correcciones al enunciado, anotadas el 2026-08-20 para que nadie escriba el test al pie de la letra:** (1) el punto (d) —«la tabla NO tiene política RLS, assert `pg_class.relrowsecurity = false`»— **ya es falso a propósito**: la migración `0140_cortex_owner_rls` (ADR 0156, 2026-08-19) le puso `ENABLE` + `FORCE` + policy de eje _owner_ a `cortex_affect_snapshots` y a otras cuatro tablas del córtex, así que hoy `relrowsecurity` es **true** y quien lo cubre es `tests/integration/test_cortex_owner_rls.py`. Un test escrito según este enunciado saldría rojo y tendría razón el código. (2) `downgrade -1` es relativo a la **cabeza** del árbol, no a la 0093: con la cabeza en `0143` no deshace esta migración. Ánclalo por nombre a `0092_cortex_threads` — [`gotchas/alembic-round-trip-anclado-por-nombre.md`](../03-guides/gotchas/alembic-round-trip-anclado-por-nombre.md). El fichero `tests/integration/test_cortex_affect_migration.py` que este enunciado nombra **no llegó a existir**; lo que sí cubre cada mitad está listado en la nota de la FASE I.
...
- [x] **Componente `MindPanel` montado en `app/admin/cortex` (de F1)**
  - ✅ **Hecho (2026-08-19).** Lo que faltaba el 2026-07-30 era exacto y era esto: no había test de render de los diales, y el panel estaba cableado en castellano salvo el aviso honesto y el espacio PAD, así que el «ES+EN» de la aceptación no se cumplía. Ambas cosas cerradas:
    - `apps/admin-panel/components/cortex/mind-panel.tsx` (nuevo): diales PAD + drives + aviso honesto, **juntos en un solo componente a propósito** — mientras el aviso y los diales eran piezas que la pantalla montaba una al lado de la otra, montar diales sin aviso era un olvido de una línea. Ahora no hay forma de pedir unos sin el otro.
    - Montado en la **segunda columna** de `apps/admin-panel/app/admin/cortex/page.tsx` (`data-testid="cortex-second-column"`, `columns={1}`) y también en la vista completa `app/admin/cortex/mind/page.tsx`.
    - **ES+EN**: namespace `cortexMind` en `lib/i18n/dictionary.ts` (33 claves) — `mind/page.tsx`, `mind/affect-panel.tsx` y el nuevo componente quedan a CERO infractores en `scripts/check-i18n.mjs` y **salen de su `ATTR_ALLOWLIST`**, que es lo que impide la regresión (la guarda baja de 211 a 200 atributos pendientes).
    - Tests: `components/cortex/mind-panel.test.tsx` (9 — aviso siempre presente incl. sin estado, nota bilingüe del backend, respaldo del diccionario en los dos idiomas, los cuatro diales con su número Y la anchura de su barra, rótulos ES/EN) + `app/admin/cortex/mind/pad-dials.test.tsx` (2, el dato de `/mind` llegando a la pantalla) + `app/admin/cortex/second-column.test.tsx` (5, el montaje en el chat, incluido que un no-owner no ve la columna).
  - **Desviación consciente frente al enunciado original** (que pedía un único componente con TODO): el `MindPanel` lleva el estado VIVO (diales, drives, aviso). El espacio PAD 2D con estela (`components/cortex/mind-pad-space.tsx`), la curva de mood y el mapa de episodios con `appraisal_reason` siguen en la vista completa `/admin/cortex/mind` y **no se duplican** en la columna estrecha del chat: son histórico, piden ancho, y meterlos en 22 rem los haría ilegibles sin añadir información que el owner necesite mientras conversa. Las tres superficies existen y están montadas; lo que cambia es dónde.
...
- [ ] **Suite completa F2 en verde + lint/type**
  ...
  - ↑ **Corrección parcial (2026-08-19, desde la casilla del `MindPanel`):** el segundo motivo ya NO se sostiene — el Panel de Mente pasó al diccionario (`cortexMind`, ES+EN) y `check-i18n` lo tiene a cero, y el aviso honesto viaja pegado a los diales en el propio componente, así que también está en la segunda columna del chat. **Esta casilla NO se marca**: siguen abiertos el `alembic check` y el test humano del dial vía WS, que es lo que de verdad la bloquea.
  - ↑↑ **Reescrita el 2026-08-20: de los tres motivos queda UNO, y de los tres el que se citaba primero era falso.** Esta casilla decía «bloqueada por un humano» sobre tres cosas y **dos ya no lo estaban**. Lo que sigue abierto está en una sola línea al final.
    - **«`alembic check` no es ejecutable (la imagen de runtime no trae alembic)» era falso.** `alembic` **1.18.4** está instalado en el venv del repo (`.venv/Scripts/alembic.exe`) y `tests/integration/test_alembic_autogenerate_clean.py` ya lo invoca por API dentro de la suite. Nunca hizo falta la imagen de runtime. Ejecutado contra una BD **desechable** (`agentic_cortex_f2`, puerto 15432 — la BD del stack no se tocó):

      | Comando                                 | Resultado real                                                                                                     |
      | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
      | `alembic upgrade head`                  | ✅ exit 0, 143 migraciones aplicadas desde cero                                                                    |
      | `alembic downgrade 0092_cortex_threads` | ✅ exit 0, **51 downgrades en 11 s**; `to_regclass('cortex_affect_snapshots')` → NULL                              |
      | `alembic upgrade head` (vuelta)         | ✅ exit 0, **51 upgrades en 12 s**; la tabla vuelve con sus 14 columnas y sus 3 índices                            |
      | drift de autogenerate sobre la tabla F2 | ✅ **0 items** sobre `cortex_affect_snapshots` y **0** sobre cualquier `cortex_*`, dentro de un diff global de 178 |
      | `alembic check` (el comando literal)    | ❌ exit 1 — **y no por drift de F2**                                                                               |

      Se ancló por **nombre** (`0092_cortex_threads`) y nunca con `downgrade -1`, que es relativo a la cabeza: ver [`gotchas/alembic-round-trip-anclado-por-nombre.md`](../03-guides/gotchas/alembic-round-trip-anclado-por-nombre.md). El enunciado de esta casilla dice `downgrade -1` y **está mal escrito**: con la cabeza en `0143` ese `-1` no toca la 0093.

    - **Por qué `alembic check` sale rojo, y por qué no es deuda de F2.** Revienta con `sqlalchemy.exc.NoReferencedTableError: Foreign key associated with column 'incoming_webhook_configs.project_id' could not find table 'projects'`, porque `apps/api-server/migrations/env.py` importa **un solo módulo** de la capa de datos (`api_server.db.models`) y con él `Base.metadata` tiene **34 tablas de 84** (medido el 2026-08-20; el paquete entero da 84). Es un hallazgo **anterior y ajeno** a esta fase, ya escrito en el docstring de `tests/integration/test_alembic_autogenerate_clean.py` («y una nota sobre `env.py`, que es un hallazgo aparte»), que además razona por qué arreglarlo cambia el autogenerate para todo el mundo y merece su propio cambio con su propia revisión. Y aunque se arreglase, el diff global trae **178 items** de deriva histórica (nombres de índice/FK, `TEXT` frente a `String(n)`, índices por partición del ADR 0151): `alembic check` no puede dar verde hoy para **ningún** plan. **Exigirlo aquí es pedirle a F2 que cierre deuda de otro**, así que deja de contar como bloqueo de esta casilla.
      - ↑ **Actualización del mismo día (2026-08-20, más tarde): la deuda de otro se pagó, y el `alembic check` de esta casilla YA es comprobable.** Las dos afirmaciones de arriba envejecieron en horas y conviene decir cuál era cuál. (1) El `env.py` se arregló (commit `bc521ad4`): recorre el paquete entero, **53 módulos y 84 tablas**, así que el `NoReferencedTableError` no vuelve y el comando por fin **da un veredicto** en vez de reventar. (2) El veredicto fue **162 items de deriva en 23 tablas**, y una ola de cuatro carriles lo bajó a **22 items en 16 tablas**, declarando en el modelo los índices y las FK que las migraciones habían creado. **Ninguno de los 22 es de una tabla `cortex_*`**, así que la conclusión de esta casilla —el drift de F2 es cero— no cambia: lo que cambia es que ahora se puede _demostrar_ con el comando literal en vez de razonando alrededor de su traceback. Lo que sigue siendo cierto: `alembic check` **sale 1** mientras queden esos 22 items, así que «`alembic check` sin drift» tal como está escrito en el enunciado de abajo **no se puede exigir a esta fase** — lo que sí se puede exigir, y está en verde, es que el diff no toque ninguna tabla de F2. Los 22 que quedan están inventariados uno a uno, con su familia y su motivo, en `tests/integration/test_alembic_autogenerate_clean.py::REMAINING_DRIFT_2026_08_20`, y de ellos **7 piden una migración que firma el operador** (no se arreglan en el modelo). La trampa de fondo quedó escrita en [`gotchas/alembic-metadata-a-medias-propone-borrar-lo-que-no-ve.md`](../03-guides/gotchas/alembic-metadata-a-medias-propone-borrar-lo-que-no-ve.md).
    - **Lo que sí acredita ya una máquina** (corrido el 2026-08-20, no inferido): unit afectivos `tests/unit/test_cortex_affective.py test_cortex_affective_calibration.py test_cortex_affect_model.py test_cortex_affect_policy.py test_cortex_affect_enqueue.py test_cortex_affect_redis_url.py test_cortex_affect_worker_settings.py test_cortex_platform_affect.py` → **118 passed en 15,5 s**; frontend del córtex (`components/cortex`, `lib/cortex-affect.test.ts`, `app/admin/cortex`) → **99 passed / 13 ficheros**, incluidos los 9 del `MindPanel`, los 2 de `pad-dials` y los 2 de `honesty-i18n` (ES y EN). El resto de la aceptación tiene dueño automático y no hace falta comprobarlo a mano: el drift modelo↔migración lo cubre `tests/unit/test_cortex_affect_model.py` (DDL contra DDL, sin BD), la idempotencia del UNIQUE parcial `tests/integration/test_cortex_affect_task.py::test_distill_is_idempotent_per_turn` (contra PostgreSQL real), la ejecución real del `downgrade()` de la 0093 `tests/integration/test_cortex_threads_migration.py` (baja hasta `0091_system_owner_f0`, o sea pasa por encima de la 0093) y la RLS + el aislamiento cross-owner `tests/integration/test_cortex_owner_rls.py`.
    - **⏳ LO ÚNICO QUE QUEDA DE UN HUMANO, y son cinco minutos:** abrir `http://localhost:8080/admin/cortex/mind`, hablarle al córtex desde `http://localhost:8080/admin/cortex` en otra pestaña, y ver que **algún dial PAD se mueve sin recargar** (1-20 s después: el appraisal es asíncrono por diseño, ADR 0075), con el aviso honesto siempre a la vista, y repetir el vistazo con el idioma en **EN**. El procedimiento exacto —URLs, cómo confirmar en DevTools que el frame llegó por el WS y no por el polling, criterio de pass/fail y los cuatro comandos de diagnóstico si falla— está escrito en [`docs/03-guides/human-tests/cortex-f2-afectivo.md`](../03-guides/human-tests/cortex-f2-afectivo.md), para que no haya que reconstruirlo.
      - **Y un aviso que ahorra media hora de desconcierto**: las imágenes desplegadas en esta máquina son del **2026-08-13** y la BD viva está en `0139_executions_steps_rollup` (la cabeza es `0143`). Comprobado con `grep` sobre el bundle del contenedor: `/admin/cortex/mind` **sí** está desplegado con sus diales, así que **el test se puede hacer hoy sin reconstruir**; pero `cortex-second-column` y `cortex-mind-panel` —el `MindPanel` que la casilla de arriba montó junto al chat el 2026-08-19— **no están en esa imagen**. Quien abra `/admin/cortex` esperando diales a la derecha del hilo no los verá, y no será un fallo: será una imagen de hace una semana. Para validar también la segunda columna hay que reconstruir el panel antes.
    - **Esta casilla NO se marca `[x]`**: su aceptación incluye mirar una pantalla, y eso no lo puede acreditar un agente.
