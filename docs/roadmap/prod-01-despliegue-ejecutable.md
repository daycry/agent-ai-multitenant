...
1. **Registro de imágenes sin decidir**: `ghcr.io/agentic-platform` presupone una organización GitHub con packages habilitados; si la empresa exige registro corporativo, task_prod01_03 cambia de destino. Mitigación: `PLATFORM_REGISTRY` ya es parametrizable; decidir el registro en la primera semana.
   - **Resuelto el 2026-08-21**, y la premisa era falsa: la organización `agentic-platform` existe pero es **ajena a este repositorio**, y el `GITHUB_TOKEN` de Actions sólo publica en el namespace del dueño. El destino era inalcanzable por construcción y no se vio hasta que el workflow corrió por primera vez en `master` (los 14 builds en rojo). Los workflows derivan ahora el namespace de `${{ github.repository_owner }}`; lo vigila `tests/unit/test_ghcr_namespace_is_pushable.py`. Detalle en el [ADR 0148, §«Corrección operativa»](../05-architecture-decisions/0148-distribucion-imagenes-runtime-por-digest.md).
