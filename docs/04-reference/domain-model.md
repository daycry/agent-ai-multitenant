...
| `SkillCategory`             | `backend`, `frontend`, `devops`, `qa`, `research`, `docs`                                                                                                                           | Categoría del catálogo de skills (alineada al seed; CHECK `ck_skills_category`, migración 0078).                                                                                                      |
...
| `PlanStatus`                | `draft`, `pending_approval`, `pending_second_approval`, `approved`, `in_progress`, `blocked`, `pending_human_validation`, `completed`, `cancelled`, `rejected`, `archived`          | Ciclo de vida del plan (firma simple/doble). `rejected` NO es terminal: aceptar tareas correctivas lo reactiva — `rejected → in_progress` (ADR 0107).                                                 |
...
| `DocumentStatus`            | `pending`, `pending_scan`, `processing`, `indexed`, `failed`                                                                                                                        | Ingestión de documentos en una KB. `pending_scan` = AV inalcanzable en modo fail-closed (ADR 0105): no se indexa; el sweep lo re-encola y el operador recibe `antivirus_unreachable` si dura >15 min. |
...
| `worker_config`                                        | `jsonb DEFAULT '{}'`                          | Overrides de cola / runtime template. Subclave `git_policies` (ADR 0072): `branch_push_mode`, `plan_validation_mode`, `push_policy` del flujo git del plan.             |
| `repository_config`                                    | `jsonb NULL`                                  | URL del bare repo + ramas (metadatos).                                                                                                                                  |
| `git_config`                                           | `jsonb NULL`                                  | ADR 0072: `provider`/`remote_url`/`default_branch`/`auth_mode` del remoto. **Sin secreto** (el PAT/clave SSH va a Vault `projects/{id}/git`). `PUT /projects/{id}/git`. |
