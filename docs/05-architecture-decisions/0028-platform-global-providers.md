...
- **No se implementa nada de esto ahora.** Este ADR fija la decisión;
  los planes 08 / 09 / 11 implementan sus partes con esta decisión
  como guía.
- El `DEFAULT_AI_PRICE_CATALOG` en código sigue siendo la fuente de
  verdad mientras Plan 11 no llegue. Cuando llegue, Plan 11 incluye
  una migration que vuelca el catálogo a la nueva tabla y borra el
  constant.
- La página `/admin/llm-providers` y la de `/admin/llm-models` son
  parte de Plan 11 (UI) o de un follow-up del Plan 06.6 si Plan 11 se
  retrasa.
- Vault paths sugeridos:
  - Auth: `vault:secret/data/platform/auth/<provider_id>` (e.g.
    `.../auth/azure-ad-empresa`).
  - LLM: `vault:secret/data/platform/llm/<provider_id>` (e.g.
    `.../llm/claude-sdk` ó `.../llm/azure-foundry`).
  - **NO** se usan rutas tenant-scoped (`vault:secret/data/<tenant>/...`)
    para estos providers — eso queda reservado para los MCP servers
    per-proyecto (ADR 0025).
